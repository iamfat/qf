...
final class ORM_Pool
{
    ...
    public static $cleanup_timeout = 0;
    ...
    public static function get($oname, $oid)
    {
    ...
    public static function set($oname, $oid, $object, $ref_count = 0)
    {
        $key = self::key($oname, $oid);
    ...
    public static function ref($oname, $oid)
    {
        $key = self::key($oname, $oid);
        $o = self::$POOL[$key];
        if (isset($o)) {
    ...
    public static function unref($oname, $oid)
    {
        $key = self::key($oname, $oid);
        $o = self::$POOL[$key];
        if (isset($o)) {
    ...
    public static function release($oname, $oid = null)
    {
        ...
        $key = self::key($oname, $oid);
        $o = self::$POOL[$key];
        if ($object === null || $object === $o->object) {
            unset(self::$POOL[$key]);
        }
...
{
    public const RELA_PREFIX = '_r_';
    public const OBJ_NAME_SUFFIX = '_name';
    public const OBJ_ID_SUFFIX = '_id';
    public const JSON_SUFFIX = '_json';
    ...
    public function get($name, $original = false)
    {

        if (!$original && array_key_exists($name, $this->_update)) {
        ...
        if (isset($object_keys[$name])) {
            ...
            if ($attr_oname && self::class_exists($attr_oname)) {
                ...
                $pooled_object = null;
                if ($attr_oid) {
                    $pooled_object = ORM_Pool::get($attr_oname, $attr_oid);
                }
                if ($pooled_object) {
    ...
    public function &to_assoc(array $keys, array $replaced_keys = null)
    {
        $arr = array();
        foreach ($keys as $key) {
        ...
        if ($replaced_keys) {
            $arr = array_combine($replaced_keys, $arr);
        }
        return $arr;
    }
    ...
    public function save($overwrite = false)
    {
        ...
        */
        ...
        if (!in_array($this->name(), Config::get('nocache.models', []))) {
    ...
    public function cache_name($id = null)
    {
    ...
    public function delete()
    {
        ...
        if ($this->id) {
            ...
            $db->query('DELETE IGNORE FROM `%s` WHERE `id`="%s"', $name, $id);
    ...
    public function connect($object, $type = null, $approved = false)
    {
        ...
        $real_name1 = self::real_name($name1);
        $real_name2 = self::real_name($name2);
        ...
        $db->prepare_table(
            ...
            array(
                ...
                'relations' => array(
                    $conn_table . '_ibfk_1' => array('column' => 'id1', 'ref_table' => $real_name1, 'ref_column' => 'id', 'update' => 'cascade', 'delete' => 'cascade'),
                    $conn_table . '_ibfk_2' => array('column' => 'id2', 'ref_table' => $real_name2, 'ref_column' => 'id', 'update' => 'cascade', 'delete' => 'cascade'),
                ),
            ),
        );
    ...
    public function disconnect($object, $type = null, $approved = null)
    {
        ...
        if ($db->table_exists($conn_table)) {
            ...
            $db->query('DELETE IGNORE FROM `%s`' . $where, $conn_table);
    ...
    public function enum_connections($object)
    {
    ...
    public static function counterpart($type)
    {
        static $dict, $dict_r;
        if (null === $dict) {
    ...
    public function connected_with($object, $type = '', $approved_only = false)
    {
    ...
    public static function &schema($object)
    {
        ...
        if (!$schema[$name]) {
            ...
            if (count($class_schema['fields']) <= 0) {
                return null;
            }
    ...
    protected function encode_objects(&$data)
    {
        ...
        $schema = self::schema($this);
        //如果schema中存在object 则从object_id/object_name获得实例化object
        if ($schema && count($schema['object_keys']) > 0) {
            foreach ($schema['object_keys'] as $key => $val) {
    ...
    public function fetch_data($criteria = null, $no_fetch = false)
    {
        ...
        if ($id && count($real_names) > 0) {
            ...
            if ($delete_me) {
                // 如果父类数据不存在 删除相关数据
                foreach ($real_names as $rname) {
                    if ($delete_me_until == $rname) {
                        break;
                    }
                    $db = self::db($rname);
                    $db->query('DELETE IGNORE FROM `%s` WHERE `id`=%d', $rname, $id);
                }
    ...
    public static function factory($name, $criteria = null, $no_fetch = false)
    {
    ...
    public static function parent_name($name)
    {
        static $pn_cache, $suffix;

        if (!class_exists($name . MODEL_SUFFIX)) {
            return null;
        }
    ...
    public static function real_name($name)
    {
        static $rn_cache;
    ...
    // 获得对象每个对象属性及对应的真实属性名
    public static function &object_keys($name)
    {
        static $ok_cache;
        ...
        if (!isset($ok_cache[$name])) {
            ...
            foreach (self::real_names($name) as $rname) {
                ...
                if (isset($schema['object_keys'])) {
                    $object_keys += $schema['object_keys'];
                }
            }
    ...
    public static function &json_keys($name)
    {
        static $jk_cache;
        ...
        if (!isset($jk_cache[$name])) {
            ...
            foreach (self::real_names($name) as $rname) {
                $schema = self::schema($rname);
                if (isset($schema['json_keys'])) {
                    $json_keys += $schema['json_keys'];
                }
            }
    ...
    // 获得对象每个field对应的真实表名
    public static function field_to_real_name($name)
    {
        static $fr_cache;
    ...
    public static function real_names($name)
    {
        static $rn_cache;
    ...
    public static function fields($name, $refresh = false)
    {
        return self::db($name)->table_fields(self::real_name($name), $refresh);
    }

    public static function indexes($name, $refresh = false)
    {
    ...
    public static function refetch($object)
    {
    ...
    public static function destroy($name)
    {
        ...
        if ($name != $real_name) {
            unset(self::$_table_prepared[$name]);
        }
    }
